...
class NoAccessKeyID(AmazonException):
    pass


class NoSecretAccessKey(AmazonException):
    pass


class BadLocale(AmazonException):
    pass
...
class Pyzon(object):
    ...
    def getAccessKeyID(self):
        ...
        if not self._access_key_id:
            raise NoAccessKeyID, (
                "Please get the license key from http://aws.amazon.com/")
    ...
    def setLocale(self, locale):
        ...
        """
        locale = locale.upper()
        if not locale in supportedLocales:
            raise BadLocale, ("Unsupported locale. Locale must be one of: %s" % ', '.join(
                [x for x in supportedLocales.keys()]))
        self.setUrlHost(supportedLocales[locale])
        return True
    ...
    def _buildUrl(self, params):
        ...
        string_to_sign = "GET\n%s\n/onca/xml\n%s" % (
            self._urlhost, urlencoded_reqs)
        hmac_digest = hmac.new(
            self._secret_access_key, string_to_sign, hashlib.sha256).digest()
        base64_encoded = base64.b64encode(hmac_digest)
        signature = urllib2.quote(base64_encoded)
        url = "http://%s/onca/xml?%s&Signature=%s" % (
            self._urlhost, urlencoded_reqs, signature)
        return url
    ...
    def _sendHttpRequest(self, url):
        ...
        if self._proxy_host:
            proxy_handler = urllib2.ProxyHandler(
                {'http': 'http://%s:%s/' % (self._proxy_host, self._proxy_port)})
